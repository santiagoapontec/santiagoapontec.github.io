...
> Objects can take many different forms — They can be numbers, people, letters, and even other sets. We call these objects the _elements_ of a set.

We describe a set by listing all the objects^[We will see in just a minute that there's one exception of this, which is when our sets are infinite] in the set and enclosing the list in curly braces -> {}.
...
You see how the pattern we are following is easily understood by a reader. You yourself will have deduced that the set countinues indefinitely by adding _one_ to the previous number.
...
Coming back to known mathematical sets^[Meaning sets you will often hear referenced], if we add the number _zero_ to the set of natural numbers, we get the set of whole numbers.

And if we have a set consiting of the numbers _zero through nine_ we call that the set of digits - since these are the numbers from which we construct bigger whole numbers. Notice that the whole number 434 (_Four hundred thirty-four_ ) are just the digits 4, 3 and 4.
...
1.  _Classifying information_, so that we know what rules apply to what group (say, integers vs whole numbers).
2.  _Defining relationships_, since sets make it easy to visualize how groups overlap or differ (with tools like venn diagrams)
3.  _Base foundation_, since set theory helps glue much of the concepts of higher-level mathematics like probability, functions, or geometry.^[Hopefully we will see this applied later on. But consider for instance the probability problem "*What is the chance of rolling an even number or a number greater than four?* - to solve this we would basically just be counting sets (the set of even numbers and the set of the numbers greater than 4).]
...
In a number line, we call the distance between one number and another the _unit lenght_.
...
The _comparisson property_ tells us that when comparing two whole numbers A and B, only one of three possibilities is true.
